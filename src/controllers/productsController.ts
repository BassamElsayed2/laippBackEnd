...
export const getProducts = async (
  ...
  try {
    ...
    if (search) {
      whereConditions.push('(title LIKE @search OR name_ar LIKE @search OR name_en LIKE @search)');
...
export const createProduct = async (
  ...
  try {
    ...
    await pool
      ...
      .input('limited_time_offer', sql.Bit, limited_time_offer || false)
      .query(`
    ...
    if (attributes && attributes.length > 0) {
      for (const attr of attributes) {
        await pool
          ...
          .input('attribute_value', sql.NVarChar, attr.attribute_value)
          .query(`
